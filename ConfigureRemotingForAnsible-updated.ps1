...
Function Disable-GlobalHttpFirewallAccess {
    Write-Verbose "Disabling global HTTP firewall access"
    ...
    If ($matching_rules) {
        If ($matching_rules -isnot [Array]) {
            ...
            $matching_rules.Enabled = $false
        }
        Else {
            Write-Verbose "Found multiple existing HTTP firewall rules..."
            $matching_rules[0].Enabled = $false
            $matching_rules[1].Enabled = $false
        }
    }
    Write-Verbose "HTTP firewall rules disabled"
}
...
Else {
    Write-Verbose "SSL listener already present. Forcing new SSL cert on new listener.."
...
If ($GlobalHttpFirewallAccess) {
    Disable-GlobalHttpFirewallAccess
}
...
# Remove AIB script if still in the image
